...
// Enlarge BBox to follow displacement -- prevents the object to be discarded
// when moved at the edge of the viewport
...
#include "include\\PS_HWSelection.fxh"
#include "include\\PS_SolidColor.fxh"
#include "include\\GS_FatLine.fxh"
#include "include\\GS_FatPoint.fxh"
...
<
	...
	bool handlesConsolidatedGeometry = false;
>
{
    pass pNormal
    {
...
technique10 NormalView_Selection
<
	// We do not need transparency for this technique
	int isTransparent = 0;

	// Use the effect to also render non material items (ie. wireframe) so they
	// will follow the displacement
	bool overridesNonMaterialItems = true;
...
>
{
	...
	>
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_4_0, PS_SolidColor()));
	}
	...
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(CompileShader(gs_4_0, GS_FatLine()));
		SetPixelShader(CompileShader(ps_4_0, PS_SolidColor()));
	}
	...
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(CompileShader(gs_4_0, GS_FatPoint()));
		SetPixelShader(CompileShader(ps_4_0, PS_SolidColor()));
	}
	...
	>
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_4_0, PS_HWSelection()));
	}
	...
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(CompileShader(gs_4_0, GS_FatLine_ForHWSelection()));
		SetPixelShader(CompileShader(ps_4_0, PS_HWSelection()));
	}
	...
	{
		SetVertexShader(CompileShader(vs_4_0, VS_Wave()));
		SetGeometryShader(CompileShader(gs_4_0, GS_FatPoint_ForHWSelection()));
		SetPixelShader(CompileShader(ps_4_0, PS_HWSelection()));
	}
}
