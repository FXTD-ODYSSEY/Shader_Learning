...
float3 lightPosition : ViewPosition  
< 
	string UIWidget = "None"; 
>;
...
lightOut CalculateLight	(	bool lightEnable, int lightType, float lightAtten, float3 lightPos, float3 vertWorldPos, float3 lightColor, float lightIntensity,
							float3 lightDir, float lightConeAngle, float lightFallOff, float4x4 lightViewPrjMatrix, uniform Texture2D lightShadowMap,
...
{ 
	...
	LightToVertex = IN.worldPosition.xyz - lightPosition.xyz;
